...
export default class Header extends React.Component {
  render() {
    ...
    let is_white_header =
      _.get(page, "frontmatter.white_header", null) || false;
    let is_logo_light = false;
    if (
      is_white_header ||
      _.get(page, "frontmatter.layout", null) === "product"
    ) {
      is_logo_light = true;
    }
    return (
        ...
        <header className="header">
          <nav
            className={classNames("nav", {
              "nav--light": is_white_header,
              "nav--dark": is_white_header !== true,
            })}
          >
            <div
              className="nav__logo"
              {...(is_logo_light && _.get(site, "data.config.logo_light", null)
                ? {
                    "data-original": withPrefix(
                      _.get(site, "data.config.logo_light", null)
                    ),
                  }
                : null)}
              {...(_.get(site, "data.config.logo_dark", null)
                ? {
                    "data-dark": withPrefix(
                      _.get(site, "data.config.logo_dark", null)
                    ),
                  }
                : null)}
            >
              <Link href={withPrefix("/")}>
                {is_logo_light ? (
                  <Picture
                    {...this.props}
                    image={_.get(site, "data.config.logo_light", null)}
                  ...
                  <Picture
                    {...this.props}
                    image={_.get(site, "data.config.logo_dark", null)}
                    cssClass={"nav__logo-image"}
                    alt={"Site logo"}
                  />
                )}
            ...
            <ul className="nav__menu">
              {_.map(
                _.get(site, "data.config.main_menu", null),
                (item, item_idx) => {
                  let section =
                  ...
                  return (
                    <React.Fragment key={item_idx + ".1"}>
                      <li key={item_idx} className="nav__menu-item">
                        <Link
                          href={withPrefix(_.get(item, "url", null))}
            ...
            <div className="nav__right">
              ...
              <div
                ...
                {...(_.get(site, "data.config.hamburger_background_image", null)
                  ? {
                      style: toStyleObj(
                        "background-image: url('" +
                          withPrefix(
                            _.get(
                              site,
                              "data.config.hamburger_background_image",
                              null
                            )
                          ) +
                          "')"
              ...
              >
                <div className="hamburger__options">
                  ...
                  <div className="">
                    <span className="snipcart-items-count" /> products |{" "}
                      ...
                      return (
                        <React.Fragment key={item_idx + ".1"}>
                          <li key={item_idx} className="hamburger__nav-item">
                            <Link
                              href={withPrefix(_.get(item, "url", null))}
                              className={classNames("hamburger__nav-link", {
