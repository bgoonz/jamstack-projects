window.onGatsbyRouteUpdate = function () {
  ...
  let originalHeaderColor = nav.classList.contains("nav--dark")
    ? "nav--dark"
    : "nav--light";
  ...
  function moveHandler(event) {
    ...
    if (target.isActive) {
      ...
      if (newValue <= 0 && newValue >= -1 * target.clientWidth + 300)
        target.style.transform = `translateX(${newValue}px)`;
      startScroll();
    }
  }
