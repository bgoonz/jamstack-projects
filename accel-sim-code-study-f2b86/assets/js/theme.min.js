(() => {
  ...
  function e(t) {
    ...
    if (e.length > 0) {
      $(".toctree .current").removeClass("current"),
        e.addClass("current"),
        e.closest(".level-1").parent().addClass("current");
      for (var n = 1; n <= 11; n++)
        e.closest(".level-".concat(n)).addClass("current");
    }
  }
  function n(t, e) {
  ...
  $(window).bind("hashchange", function () {
    ...
    $(document).on("scroll", function () {
      ...
      for (var a = 0; a < n.length; a++)
        t > n[a].offset &&
          (a < n.length - 1
            ? t < n[a + 1].offset &&
              (1 == n[a].level ? e(location.pathname) : e("#" + n[a].id))
            : e("#" + n[a].id));
    }),
    ...
    location.pathname == "".concat(ui.baseurl, "/search.html") &&
      $.ajax("".concat(ui.baseurl, "/data.json"))
        .done(function (e) {
          var n = new URL(location.href).searchParams.get("q");
          new URL(location.href).searchParams.get("lang") || ui.lang,
            $("input[name='q']").val(n);
          var a = [],
            o = new RegExp();
          try {
            ...
            return (
              ...
              $(".search-results .summary").html(
                ui.i18n.search_results_not_found
              ),
              ...
              if (i || u) {
                ...
                if (u) {
                  var f = u.index - 100,
                    ...
                    g = "...";
                  f < 0 && ((p = ""), (f = 0)),
                    d > page.content.length &&
                      ((g = ""), (d = page.content.length)),
                    h.push(
                      '<p class="text-gray">'
                        .concat(p)
                        .concat(s(page.content, f, d))
                        .concat(g, "</p>")
                    );
                }
                a.push(
                  '<li class="border-top py-4">'.concat(h.join(""), "</li>")
                );
              }
            }
          } catch (t) {
          ...
          a.length > 0 && n.length > 0
            ? ($(".search-results .content").html(a.join("")),
              $(".search-results .summary").html(
                ui.i18n.search_results_found.replace("#", a.length)
              ))
            : ($(".search-results .content").empty(),
              $(".search-results .summary").html(
                ui.i18n.search_results_not_found
    ...
    $(".sidebar").scroll(function () {
      n("scroll", this.scrollTop),
        n("scrollTime", Date.now()),
        n("scrollHost", location.host);
    }),
    (l = new URL(location.href).searchParams.get("highlight")) &&
      ($(".markdown-body")
        .find("*")
        .each(function () {
          try {
            this.outerHTML.match(new RegExp(l, "im")) &&
              ($(this).addClass("search-result"),
              $(this)
                .parentsUntil(".markdown-body")
    ...
    $(".markdown-body :header").append(function () {
      return '<a href="#'.concat(
        this.id,
        '" class="anchor"><i class="octicon-link fa fa-link text-blue"></i></a>'
      );
    }),
    ...
    "serviceWorker" in navigator
      ? navigator.serviceWorker.register("".concat(ui.baseurl, "/sw.caches.js"))
      : r("Service Worker not supported!");
})();
