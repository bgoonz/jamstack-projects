      ...
      <figure className="mb-4">
        <Link to={`/blog/${teaser.slug}`}>
          <img
            src={
              teaser.heroImage &&
              `https:${teaser.heroImage.file.url}?w=620&h=310&fit=thumb`
            }
            alt={teaser.heroImage && teaser.heroImage.title}
          />
        </Link>
      </figure>
      <h2>
        <Link
          to={`/blog/${teaser.slug}`}
          className="text-xl text-black font-semibold no-underline hover:underline"
        >
          {teaser.title}
        </Link>
      </h2>
