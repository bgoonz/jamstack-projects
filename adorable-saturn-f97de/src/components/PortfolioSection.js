...
export default class PortfolioSection extends React.Component {
  render() {
    ...
    let projects_recent = projects_sorted.slice(
      0,
      _.get(section, "projects_number", null)
    );
      ...
      >
        <div className="container container--lg">
          {_.get(section, "title", null) && (
            <h2 className="section__title line-top">
              {_.get(section, "title", null)}
            </h2>
          )}
          ...
          >
            {_.map(projects_recent, (project, project_idx) =>
              project_idx === project_len - 1 &&
              _.get(section, "view_all_label", null) &&
              _.get(section, "view_all_url", null) ? (
                <article key={project_idx} className="cell project-card">
                  <Link
                    href={withPrefix(_.get(section, "view_all_url", null))}
                    className="project-card__view-all"
                  >
                    {_.get(project, "thumb_image", null) && (
                      <div className="project-card__image">
                        <img
                          src={withPrefix(_.get(project, "thumb_image", null))}
                          alt={_.get(project, "thumb_image_alt", null)}
                        />
                      </div>
                    )}
                    <span className="project-card__button">
                      {_.get(section, "view_all_label", null)}
                    </span>
                  </Link>
                </article>
