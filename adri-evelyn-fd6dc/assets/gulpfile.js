...
gulp.task("img", function imging() {
  return gulp
    .src("img/**/*.{png,svg,jpg,webp,jpeg,gif}")
    .pipe(imagemin())
    .on("error", (err) => {
      console.log(err.toString());
    })
...
gulp.task("thumbnails-all", function () {
  ...
  return (
    ...
    gulp
      .src("img/!(thumbnails)/*.{png,jpg,webp,jpeg}")
      .pipe(responsive({ "**/*.*": settings }))
      .pipe(gulp.dest("img/thumbnails"))
  );
});
...
gulp.task("css", function minicss() {
  return gulp
    .src("css/vendor/bootstrap-iso.css")
    .pipe(cleanCSS())
    .on("error", (err) => {
      console.log(err.toString());
    })
