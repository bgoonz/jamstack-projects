...
function findFilesInDir(startPath, ext) {
  if (!fs.existsSync(startPath)) {
    throw new Error(`[inject-stackbit-widget.js] ${startPath} was not found`);
  }
...
function injectHtmlBeforeHtml(filepath, beforeHtml, injectHtml) {
  ...
  let finalHTML = fileHTML.slice(0, index) + injectHtml + fileHTML.slice(index);
...
findFilesInDir(publishDir, "html").forEach((file) => {
  ...
  pageCount++;
  injectHtmlBeforeHtml(
    file,
    "</body>",
    `<script id="stackbit-widget-init" data-stackbit-project-id="${stackbitProjectId}" src="${stackbitWidgetUrl}"></script>`
  );
});

console.log(
  `[inject-stackbit-widget.js] done, injected script to ${pageCount} pages`
);
