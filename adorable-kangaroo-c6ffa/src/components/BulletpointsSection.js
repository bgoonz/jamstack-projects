...
export default class BulletpointsSection extends React.Component {
  render() {
    ...
    return (
      ...
      >
        {_.map(
          _.get(section, "bulletpoints", null),
          (bulletpoint, bulletpoint_idx) => (
            <section
              key={bulletpoint_idx}
            ...
            >
              <Picture
                {...this.props}
                image={_.get(bulletpoint, "image", null)}
                alt={_.get(bulletpoint, "title", null)}
                cssClass={"bulletpoint__section-image"}
              />
              <div className="bulletpoint__section-content-container">
                <h2 className="bulletpoint__section-title">
                  {_.get(bulletpoint, "title", null)}
                </h2>
